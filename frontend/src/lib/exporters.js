...
export function exportPdf(data, filenameBase) {
  ...
  // --- Render Sentiment badge (if provided) ---
  if (data.sentiment) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.setTextColor(SUBTEXT_COLOR);
    doc.text(`Sentiment: ${data.sentiment.charAt(0).toUpperCase() + data.sentiment.slice(1)}`, MARGIN, y);
    y += 20;
  }

  // --- Render Summary section ---
  renderHeading('Summary');
  renderBodyText(data.summary || 'No summary available.');
